...
ionice -c 3 nice -n 15 /usr/local/bin/tvshow-summary.py ${TVSHOW_CACHE:+-t "${TVSHOW_CACHE}"} -e "${TVSHOW_SUMMARY_PATH}"/tvshow-episodes.csv -c "${TVSHOW_SUMMARY_PATH}"/tvshow-completion.csv >>/var/log/tvshow-summary.log 2>&1
